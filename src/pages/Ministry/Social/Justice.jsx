...
const Justice = () => {
  ...
  useEffect(() => {
    ...
    axios.get(apiUrl)
      .then(response => {
        ...
        const pageWithId25 = allPages.find(page => page.id === 25);
        setJusticeData(pageWithId25);
      })
      .catch(error => {
  ...
  return (
    <div className="container">
      ...
      {justiceData && (
        <>
          <h1 className="text-center">{justiceData.title}</h1>
          <div className="mt-4" dangerouslySetInnerHTML={{ __html: justiceData.content }} />
        </>
      )}
    </div>
  );
